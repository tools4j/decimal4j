 ...
 */
public class AddBenchmark extends AbstractBinaryOpLongLongBenchmark {
	...
	protected <S extends ScaleMetrics> Decimal<S> immitableDecimals(BenchmarkState state, Values<S> values) {
		return values.immutable1.add(values.immutable2, state.truncationPolicy);
	}

	@Override
	protected <S extends ScaleMetrics> Decimal<S> mutableDecimals(BenchmarkState state, Values<S> values) {
		return values.mutable.set(values.immutable1).add(values.immutable2, state.truncationPolicy);
	}
